---
{"publish":true,"created":"2024-11-21T06:17:24.000-05:00","modified":"2025-08-17T17:05:22.811-04:00","cssclasses":""}
---

#math/algebra/inequality/equations , [[system of equations]], [[addition]], [[subtraction]]

- Elimination is a way to solve a [[system of equations]], first learn [[substitution]] if you haven't already
- In elimination we combine both [[equations\|Equations]]
# Problems
## Introduction problem
![[Files/Images/Pasted image 20240428131215.png]]
- We *can* solve this with [[substitution]], but we will immediately have to deal with [[Fraction\|Fractions]], so we'll instead look for a NICER and more ⭐elegant⭐ way to solve this
- We see that if we add both equations together, the $y$ terms will [[cancel out]].
	- We add $2x+3y=-11$ and $5x-3y=67$ to get $7x=56$
	- this is a much easier way to solve for $x$, we get $x=8$, we can plug it in to $2x+3y=-11$ to get $y=-9$
- Our [[ordered pair]] is $(8,-9)$
### Why it works
- Why can we just add equations and trust that it will give us a solution to one of the variables?
- When we add equations, we are adding the left sides and the right sides, the left sides are equal to the right sides, so really we're just adding stuff to the equation but it's still equal since the left and right sides in the other equation were equal
- Our equation remains the same, we just solve and plug it in :)

## Manipulating before adding
Sometimes we have to manipulate the equations before adding
![[Files/Images/Pasted image 20240428131944.png]]
- Here we can see that when we add the equations, no variable will cancel out
- We can multiply the second equation by $-2$ to get $-4x-6y=10$
	- Now when we add the equations the $x$ [[variable math\|variable]] *will* cancel out. How cool is that?
	- adding gives $-13y=23$
	- We have $y=-\frac{23}{13}$, sometimes there's just no avoiding [[Fraction\|Fractions]]
	- We can plug $y$ into $2x+3y=-5$ to get $2x+3\left( -\frac{23}{13} \right)=-5$
		- Solving this gives us $x=\frac{2}{13}$
	- Our [[ordered pair]] is $\left( \frac{2}{13},-\frac{23}{13} \right)$

## Subtracting the equations?
We can also subtract equations before adding them, for example:
![[Files/Images/Pasted image 20240428132850.png]]
- We can subtract the equations so the $x$'s cancel out
- this has the same effect of multiplying an equation by $-1$, and then adding
...
- We start by organizing the information, putting all the variables on one side and the constants on the other, we have:
		- $5x-2y=12$
		- $3y-9x=-22$
	- Solving for one variable looks messy, but we can multiply the first equation by $3$ and the second by $2$ so the $ys cancel out, we have
		- ${} 15x-6y=36 {}$
		- $-18x+6y=-44$
